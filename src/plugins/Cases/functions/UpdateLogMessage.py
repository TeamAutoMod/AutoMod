...
async def updateLogMessage(plugin, ctx, log_id, case, reason):
    ...
    if log_channel_id is "":
        return await ctx.send(plugin.t(ctx.guild, "no_mod_log_set", _emote="WARN"))
    ...
    try:
        ...
        return await ctx.send(plugin.t(ctx.guild, "log_not_found", _emote="WARN"))
    if msg is None:
        return await ctx.send(plugin.t(ctx.guild, "log_not_found", _emote="WARN"))
    ...
    try:
        ...
        await ctx.send(plugin.t(ctx.guild, "log_edit_failed", _emote="WARN", exc=ex))
    else:
        plugin.db.inf.update(f"{ctx.guild.id}-{case}", "reason", reason)
        await ctx.send(plugin.t(ctx.guild, "log_edited", _emote="YES", case=case))