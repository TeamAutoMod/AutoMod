...
async def init(actual_bot):
    global BOT
    global BOT_LOG
    BOT_LOG = await actual_bot.fetch_channel(int(Utils.from_master("GLOBAL_LOG_CHANNEL")))
    BOT = actual_bot
...
def init_sentry():
    dsn = Utils.from_master("SENTRY_DSN")
    sentry_sdk.init(dsn, before_send=before_send)


async def global_log(bot, text=None, embed=None):
    log_channel: discord.TextChannel = await bot.fetch_channel(int(Utils.from_master("GLOBAL_LOG_CHANNEL")))
    await log_channel.send(content=text, embed=embed)


async def guild_log(bot, text=None, embed=None):
    log_channel: discord.TextChannel = await bot.fetch_channel(int(Utils.from_master("GUILD_LOG_CHANNEL")))
    await log_channel.send(content=text, embed=embed)
