...
class Admin(BaseCog):
    ...
    async def cog_check(self, ctx):
        return await ctx.bot.is_owner(ctx.author) or ctx.author.id in Utils.from_master("BOT_ADMINS")
    ...
    async def load(self, ctx, cog):
        if os.path.isfile(f"Cogs/{cog}.py") or os.path.isfile(f"./Cogs/{cog}.py"):
            ...
            await Logging.global_log(self.bot, f"**{cog}** has been loaded by {ctx.message.author.name}.", None)
        else:
    ...
    async def unload(self, ctx, cog):
        if os.path.isfile(f"Cogs/{cog}.py") or os.path.isfile(f"./Cogs/{cog}.py"):
            ...
            await Logging.global_log(self.bot, f"**{cog}** has been unloaded by {ctx.message.author.name}.", None)
        else:
            await ctx.send(f"{RED_TICK} I can't find a cog with that name.")
    ...
    async def reload(self, ctx, cog):
        if os.path.isfile(f"Cogs/{cog}.py") or os.path.isfile(f"./Cogs/{cog}.py"):
            ...
            await Logging.global_log(self.bot, f"**{cog}** has been reloaded by {ctx.message.author.name}.", None)
        else:
            await ctx.send(f"{RED_TICK} I can't find a cog with that name.")
    ...
    async def block_server(self, ctx, guild: Guild):
        ...
        await guild.leave()
        await Logging.global_log(self.bot, f"{guild.name} (``{guild.id}``) has been blocked by {ctx.author}")
    ...
    async def reload_i18n(self, ctx, langs = None):
        try:
            if langs is None:
                langs = Utils.from_master("SUPPORTED_LANGS")
