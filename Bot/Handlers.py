...
async def on_ready(bot):
    try:
        if not bot.READY:
            log.info("[Booting Up] AutoMod is starting")
            await init_bot(bot)
            try:
...
async def on_guild_join(bot, guild: Guild):
    if guild.id in Utils.from_master("BLOCKED_GUILDS"):
        await Logging.global_log(bot, f"Someone tried adding me to blocked guild {guild.name} ({guild.id})", None)
        await guild.leave()
    else:
