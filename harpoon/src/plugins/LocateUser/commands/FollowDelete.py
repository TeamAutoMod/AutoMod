...
async def run(plugin, ctx, user_id):
    ...
    if user_id not in follows:
        return await plugin.translator.translate(ctx.guild, "not_following", _emote="WARN")

    follows.remove(user_id)
    plugin.db.follows.update(_id, "users", follows)

    await ctx.send(plugin.translator.translate(ctx.guild, "unfollowed", _emote="YES", user=user_id))