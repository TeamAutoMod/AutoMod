...
async def run(plugin, ctx, targets, reason):
    ...
    if len(targets) < 1:
        return await ctx.send(plugin.translator.translate(ctx.guild, "no_ban_then", _emote="THINK"))

    if len(targets) > 50:
        return await ctx.send(plugin.translator.translate(ctx.guild, "max_ban", _emote="WARN"))
