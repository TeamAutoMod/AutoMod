...
async def runClean(plugin, ctx, amount, check=None, before=None, after=None):
    ...
    try:
        ...
        await ctx.send(plugin.t(ctx.guild, "clean_error", _emote="WARN", exc=ex))
    else:
        if len(deleted) < 1:
            return await ctx.send(plugin.t(ctx.guild, "zero_cleaned", _emote="WARN"))
        else:
            await ctx.send(plugin.t(ctx.guild, "cleaned", _emote="YES", amount=len(deleted), plural="" if len(deleted) == 1 else "s"))
