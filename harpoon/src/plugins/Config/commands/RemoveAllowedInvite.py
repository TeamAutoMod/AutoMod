...
async def run(plugin, ctx, server):
    ...
    if str(server) not in allowed:
        return await ctx.send(plugin.translator.translate(ctx.guild, "not_whitelisted", _emote="WARN"))

    allowed.remove(str(server))
    plugin.db.update(ctx.guild.id, "whitelisted_invites", allowed)
    
    await ctx.send(plugin.translator.translate(ctx.guild, "removed_invite", _emote="YES", server=server))