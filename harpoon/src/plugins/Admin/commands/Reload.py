...
async def run(plugin, ctx, _plugin):
    ...
    if _plugin_ is None:
        ...
        try:
            ...
            await ctx.send(plugin.translator.translate(ctx.guild, "plugin_reload_failed", _emote="WARN", exc=ex))
        else:
            await ctx.send(plugin.translator.translate(ctx.guild, "plugin_reloaded", _emote="YES", plugin=str(_plugin_)))