...
class BotUtils:
    ...
    async def dmUser(self, ctx, _type, user, **kwargs):
        msg = self.bot.translator.translate(ctx.guild, f"{_type}_dm", **kwargs)
        res = ""
        state = self.bot.db.configs.get(ctx.guild.id, "dm_on_actions")
        if state is True:
            try:
