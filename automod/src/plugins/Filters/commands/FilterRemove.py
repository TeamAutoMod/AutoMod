...
async def run(plugin, ctx, name):
    ...
    if name not in filters:
        return await ctx.send(plugin.i18next.t(ctx.guild, "filter_doesnt_exist", _emote="NO"))
    
    del filters[name]
    plugin.db.configs.update(ctx.guild.id, "filters", filters)

    await ctx.send(plugin.i18next.t(ctx.guild, "filter_removed", _emote="YES", name=name))