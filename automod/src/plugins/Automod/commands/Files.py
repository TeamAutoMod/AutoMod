...
async def run(plugin, ctx, warns):
    ...
    if warns == "off":
        ...
        await ctx.send(plugin.t(ctx.guild, "automod_feature_disabled", _emote="YES", what="anti-files"))
    else:
        try:
            warns = int(warns)
        except ValueError:
            await ctx.send(plugin.t(ctx.guild, "invalid_automod_feature_param", _emote="WARN", prefix=plugin.bot.get_guild_prefix(ctx.guild), command="files <warns>", off_command="files off"))
        else:
            if warns < 1:
