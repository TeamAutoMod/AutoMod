...
async def run(plugin, ctx, guild_id):
    ...
    if str(guild_id) not in allowed:
        return await ctx.send(plugin.t(ctx.guild, "not_whitelisted", _emote="WARN", server=guild_id))

    allowed.remove(str(guild_id))
    plugin.db.configs.update(ctx.guild.id, "whitelisted_invites", allowed)
    
    await ctx.send(plugin.t(ctx.guild, "removed_invite", _emote="YES", server=guild_id))