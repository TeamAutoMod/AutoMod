...
async def run(plugin, ctx, mentions):
    ...
    if mentions == "off":
        ...
        await ctx.send(plugin.t(ctx.guild, "automod_feature_disabled", _emote="YES", what="max-mentions"))
    else:
        try:
            mentions = int(mentions)
        except ValueError:
            await ctx.send(plugin.t(ctx.guild, "invalid_automod_feature_param", _emote="WARN", prefix=plugin.bot.get_guild_prefix(ctx.guild), command="mentions <mentions>", off_command="mentions off"))
        else:
            if mentions < 4:
