...
async def run(plugin, ctx, lines):
    ...
    if lines == "off":
        ...
        await ctx.send(plugin.t(ctx.guild, "automod_feature_disabled", _emote="YES", what="max-lines"))
    else:
        try:
            lines = int(lines)
        except ValueError:
            await ctx.send(plugin.t(ctx.guild, "invalid_automod_feature_param", _emote="WARN", prefix=plugin.bot.get_guild_prefix(ctx.guild), command="lines <lines>", off_command="lines off"))
        else:
            if lines < 6:
