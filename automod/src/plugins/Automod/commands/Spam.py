...
async def run(plugin, ctx, warns):
    ...
    if warns == "off":
        ...
        return await ctx.send(plugin.t(ctx.guild, "spam_off", _emote="YES"))
        ...
        await ctx.send(plugin.t(ctx.guild, "warns_set", _emote="YES", warns=warns, what="they spam more than 10 messages within the last 10 seconds"))
