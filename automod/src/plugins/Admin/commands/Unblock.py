...
async def run(plugin, ctx, guild_id):
    ...
    if guild_id not in blocked_guilds:
        return await ctx.send(plugin.i18next.t(ctx.guild, "not_blocked", _emote="WARN"))
    ...
    await ctx.send(plugin.i18next.t(ctx.guild, "unblocked_guild", _emote="YES", guild_id=guild_id))