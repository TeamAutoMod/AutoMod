...
async def run(plugin, ctx, _plugin):
    ...
    if _plugin_ is None:
        ...
        try:
            ...
            await ctx.send(plugin.translator.translate(ctx.guild, "plugin_unload_failed", _emote="WARN", exc=ex))
        else:
            await ctx.send(plugin.translator.translate(ctx.guild, "plugin_unloaded", _emote="YES", plugin=str(_plugin_)))