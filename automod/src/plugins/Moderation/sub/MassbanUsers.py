...
async def massbanUsers(plugin, ctx, targets, reason):
    ...
    if failing >= len(targets):
        return await ctx.send(plugin.i18next.t(ctx.guild, "cant_ban_anyone", _emote="NO"))
    ...
    await ctx.send(plugin.i18next.t(ctx.guild, "mban_success", _emote="YES", users=banned, total=len(to_ban)))
