...
async def banUser(plugin, ctx, user, reason, log_type, ban_type, days=0):
    try:
        ...
        return await ctx.send(plugin.translator.translate(ctx.guild, "ban_failed", _emote="WARN", error=ex))
    else:
        ...
        dm_result = await plugin.bot.utils.dmUser(ctx, log_type, user, guild_name=ctx.guild.name, reason=reason)
        await ctx.send(plugin.translator.translate(ctx.guild, f"user_{ban_type}", _emote="YES", user=user, reason=reason, case=case, dm=dm_result))
