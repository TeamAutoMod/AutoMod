...
async def run(plugin, ctx, join, age):
    if ctx.guild.id in plugin.running_cybernukes:
        return await ctx.send(plugin.t(ctx.guild, "cybernuke_running", _emote="NO"))
    ...
    if len(targets) < 1:
        return await ctx.send(plugin.t(ctx.guild, "no_targets", _emote="NO"))
    if len(targets) > 100:
        return await ctx.send(plugin.t(ctx.guild, "too_many_targets", _emote="NO"))
    ...
    for i, t in enumerate(targets):
        ...
        if not Permissions.is_allowed(ctx, ctx.author, t):
            ...
            try:
                ...
                await ctx.send(plugin.t(ctx.guild, "ban_failed", _emote="NO", error=ex))
            else:
                banned += 1
    ...
    plugin.running_cybernukes.remove(ctx.guild.id)
    await ctx.send(plugin.t(ctx.guild, "users_cybernuked", _emote="YES", banned=banned, total=len(targets), case=case))