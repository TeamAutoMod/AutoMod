...
async def caseClaim(plugin, ctx, case):
    ...
    if _case["moderator_id"] == str(ctx.author.id):
        return await ctx.send(plugin.t(ctx.guild, "case_already_owned", _emote="WARN"))

    if _case["target_id"] == str(ctx.author.id):
        return await ctx.send(plugin.t(ctx.guild, "case_target", _emote="WARN"))
    ...
    await ctx.send(plugin.t(ctx.guild, "case_claimed", _emote="YES", case=case))