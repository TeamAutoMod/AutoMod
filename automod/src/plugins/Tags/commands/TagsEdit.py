...
async def run(plugin, ctx, tag, content):
    ...
    if len(content) > 1500:
        return await ctx.send(plugin.i18next.t(ctx.guild, "tag_content_too_long", _emote="NO"))

    await editTag(plugin, ctx, tag, content)

    await ctx.send(plugin.i18next.t(ctx.guild, "tag_edited", _emote="YES", tag=tag))