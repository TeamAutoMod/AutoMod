...
async def run(plugin, ctx, tag, content):
    ...
    if len(tags) < 1:
        return await ctx.send(plugin.t(ctx.guild, "no_tags", _emote="WARN"))

    _id = f"{ctx.guild.id}-{tag}"
    if not plugin.db.tags.exists(_id):
        return await ctx.send(plugin.t(ctx.guild, "tag_doesnt_exist", _emote="WARN"))

    if len(content) > 1500:
        return await ctx.send(plugin.t(ctx.guild, "tag_content_too_long", _emote="WARN"))
