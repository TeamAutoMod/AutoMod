...
async def run(plugin, ctx, users, warns, reason):
    ...
    for user in users:
        if not Permissions.is_allowed(ctx, ctx.author, user):
            await ctx.send(plugin.i18next.t(ctx.guild, "unwarn_not_allowed", _emote="NO"))

        else:
            _id = f"{ctx.guild.id}-{user.id}"
            if not plugin.db.warns.exists(_id):
                plugin.db.warns.insert(plugin.schemas.Warn(_id, 0))
                return await ctx.send(plugin.i18next.t(ctx.guild, "no_warns", _emote="NO"))
            else:
            ...
            await ctx.send(plugin.i18next.t(ctx.guild, "user_unwarned", _emote="YES", user=user, reason=reason, case=case, dm=dm_result, warns=warns))
